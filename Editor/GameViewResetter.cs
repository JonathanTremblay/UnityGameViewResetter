...
namespace GameViewResetter
{
    ...
    public class GameViewResetter : Editor
    {
        ...
        {
            if (SessionState.GetInt(_SESSION_STATE_KEY, 0) == 1) return; // If game view settings have already been reset, no need to do it again
            ...
            if (_SHOW_POSITIVE_MESSAGES && _hasMadeChange)
            {
                ...
                SessionState.SetInt(_SESSION_STATE_KEY, 1); // The game view settings have been reset, so we don't need to do it again
            }
        ...
        {
            SessionState.SetInt(_SESSION_STATE_KEY, 0); // The game view settings should be reset again
            ResetAllGameViewSettings();
        }
    }
#endif
